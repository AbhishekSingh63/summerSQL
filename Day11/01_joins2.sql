...
-- updating all tables with new records

SET FOREIGN_KEY_CHECKS = 0; -- temporarily disables foreign key constraint checking in MySQL: Now you can truncate any table, even if other tables reference it.
...
-- records of customer who placed any order or not
...
-- the sales manager wants a report showing every customer's name along with the orderid they placed.
...
select Products.ProductName, Products.Category, Suppliers.SupplierName
from Products
inner join Suppliers
on Products.SupplierID = Suppliers.SupplierID
where Products.Category="Electronics";

-- show customer names who have placed an order. arrange the result alphabetically
...
-- display every supplier along with the products supplied by them. sort supplier name in ascending order.
